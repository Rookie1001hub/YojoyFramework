...
#pragma warning disable 1591 // Missing XML comment for publicly visible type or member
        ...
        protected internal class Context
        {
            ...
            public Func<Rect, TElement[,], int, int, TElement> DrawElementXY;
            ...
            if (context.Value == null || colCount != context.Value.ColCount || rowCount != context.Value.RowCount)
            {
                ...
                if (context.Value.Attribute.DrawElementMethod != null)
                {
                    ...
                    if (error != null)
                    {
                        ...
                        drawElementMethod = entry.ParentType.FindMember()
                        .IsMethod()
                        .IsStatic()
                        .HasReturnType<TElement>()
                        .IsNamed(context.Value.Attribute.DrawElementMethod)
                        .HasParameters<Rect, TElement[,], int, int>()
                        .GetMember<MethodInfo>(out error);

                        if (error != null)
                        {
                            context.Value.ErrorMessage += tmpError + "\n\n" + error + "\n\n";
                        }
                        else
                        {
                            context.Value.DrawElementXY = (Func<Rect, TElement[,], int, int, TElement>)Delegate.CreateDelegate(typeof(Func<Rect, TElement[,], int, int, TElement>), drawElementMethod);
                        }
                    }
                    else
                    {
