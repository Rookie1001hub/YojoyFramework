...
namespace Sirenix.OdinInspector.Editor
{
    ...
    public class EnumSelector<T> : OdinSelector<T>
    {
        ...
        static EnumSelector()
        {
            ...
            if (typeof(T).IsEnum)
            {
                var memberNames = typeof(T).GetAllMembers(Flags.AllMembers);
                foreach (var member in memberNames)
                {
                    try
                    {
                        var obs = member.GetAttribute<ObsoleteAttribute>(true);
                        var lblText = member.GetAttribute<LabelTextAttribute>(true);
                        var msg = member.GetAttribute<InfoBoxAttribute>(true);
                        var hide = member.GetAttribute<HideInInspector>();

                        EnumMember val = new EnumMember();
                        val.value = Enum.Parse(typeof(T), member.Name);
        ...
        protected override void BuildSelectionTree(OdinMenuTree tree)
        {
            ...
            if (isFlagEnum)
            {
                ...
                {
                    tree.MenuItems.Insert(0, new OdinMenuItem(tree, GetNoneValueString(), new EnumMember() { value = 0, name = "None", niceName = "None", isObsolete = false, message = "" }));
                }
