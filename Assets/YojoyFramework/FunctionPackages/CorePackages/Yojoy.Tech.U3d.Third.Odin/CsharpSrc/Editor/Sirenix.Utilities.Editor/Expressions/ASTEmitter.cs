    ...
    internal class ASTEmitter : ASTVisitor
    {
        ...
        protected void ElementAccess(ASTNode ast, bool containerVisited)
        {
            ...
            else
            {
                ...
                this.actions.Add((il) => EmitMethodCall(il, method, container, isBaseAccess)); // BOOP
            }
        }
        ...
        {
            throw new SyntaxException(ast, "Null conditional operator is not supported yet");
            ...
            ////      value access
            ////      ( if value type, convert to nullable )
            ////      goto END
            ////  NULL_CASE:
            ////      load null value
            ////      goto END
            ////  CHECK:
            ////      load value from local
            ////      if notnull goto ELEMENT_ACCESS
            ////      goto NULL_CASE
            ////  END:

            //Label elementAccess = default(Label),
            //      nullCase = default(Label),
            //      check = default(Label),
            //      end = default(Label);

            //LocalBuilder checkValue = null;
            ...
            //this.actions.Add(il =>
            //{
            //    elementAccess = il.DefineLabel();
            //    nullCase = il.DefineLabel();
            //    check = il.DefineLabel();
            //    end = il.DefineLabel();

            //    checkValue = il.DeclareLocal(ast.Children[0].TypeOfValue);

            //    // set value to local
            //    il.Emit(OpCodes.Stloc, checkValue);
            ...
            //    // VALUE_ACCESS:
            //    il.Emit(OpCodes.Nop);
            //    il.MarkLabel(elementAccess);

            //    // load value from local
            //    il.Emit(OpCodes.Ldloc, checkValue);
            //});

            //// value access
            //valueAccess();

            //Type type = ast.TypeOfValue;
            //Type nullableType = type.IsValueType ? typeof(Nullable<>).MakeGenericType(type) : null;

            //ast.TypeOfValue = nullableType != null ? nullableType : type;
            ...
            //    {
            //        il.Emit(OpCodes.Newobj, nullableType.GetConstructor(new Type[] { type }));
            //    }
            ...
            //    if (nullableType != null)
            //    {
            //        var loc = il.DeclareLocal(nullableType);
            ...
            //    // if notnull goto ELEMENT_ACCESS
            //    il.Emit(OpCodes.Brtrue, elementAccess);
        ...
        protected void Invocation(ASTNode ast, bool hasVisitedChildren)
        {
    ...
    #region Operator Utilities
        ...
        private void EmitOperator(ASTNode ast, Operator op, ASTNode leftAst, ASTNode rightAst, out Type resultType, bool useAwfulImplicitCastHack = false, Type leftOverride = null, Type rightOverride = null)
        {
            ...
            if (IsAnyNumber(left) && IsAnyNumber(right))
            {
                ...
                if (IsNumberBooleanOperator(op) || IsBitwiseOperator(op))
                {
                    this.actions.Add((il) =>
                    {
        ...
        private static void EmitTypeConversion(ASTNode ast, ILGenerator il, Type from, Type to)
        {
            ...
            if (from.IsValueType)
            {
                if (to == typeof(object) || to == typeof(ValueType) || to.IsInterface)
                {
        ...
        private static void EmitMethodCall(ILGenerator il, MethodInfo method, Type instanceType, bool isBaseAccess = false)
        {
            if (method.IsStatic)
            {
