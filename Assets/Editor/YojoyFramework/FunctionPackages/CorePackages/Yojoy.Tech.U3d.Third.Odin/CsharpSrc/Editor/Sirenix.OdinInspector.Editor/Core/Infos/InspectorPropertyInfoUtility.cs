...
namespace Sirenix.OdinInspector.Editor
{
    ...
    public static class InspectorPropertyInfoUtility
    {
        ...
        private static bool TryCreate(MemberInfo member, SerializationBackend backend, bool allowEditable, out InspectorPropertyInfo result, List<Attribute> attributes)
        {
            ...
            {
                result = InspectorPropertyInfo.CreateForMember(member, allowEditable, backend, attributes);
            }
            ...
            {
                result = InspectorPropertyInfo.CreateForMember(member, false, SerializationBackend.None, attributes);
            }
        ...
        private static InspectorPropertyInfo[] CreateDefaultInspectorProperties(InspectorProperty parentProperty, Type type, bool includeSpeciallySerializedMembers)
        {
            ...
            Dictionary<InspectorPropertyInfo, float> memberOrders;
            BuildPropertyGroups(parentProperty, type, rootProperties, includeSpeciallySerializedMembers, out memberOrders);

            return PerformAndBakePostGroupOrdering(rootProperties, memberOrders);
        }
        ...
        public static InspectorPropertyInfo[] BuildPropertyGroupsAndFinalize(InspectorProperty parentProperty, Type typeOfOwner, List<InspectorPropertyInfo> rootMemberProperties, bool includeSpeciallySerializedMembers)
        {
            ...
            Dictionary<InspectorPropertyInfo, float> groupMemberOrders;
            BuildPropertyGroups(parentProperty, typeOfOwner, rootMemberProperties, includeSpeciallySerializedMembers, out groupMemberOrders);
            return PerformAndBakePostGroupOrdering(rootMemberProperties, groupMemberOrders);
        }

        public static void BuildPropertyGroups(InspectorProperty parentProperty, Type typeOfOwner, List<InspectorPropertyInfo> rootMemberProperties, bool includeSpeciallySerializedMembers, out Dictionary<InspectorPropertyInfo, float> groupMemberOrders)
        {
            if (rootMemberProperties.Count == 0)
            {
                groupMemberOrders = null;
                return;
            }
            ...
            Dictionary<InspectorPropertyInfo, InspectorPropertyInfo> removedMembers = new Dictionary<InspectorPropertyInfo, InspectorPropertyInfo>();
            ...
            foreach (var group in groups)
            {
                ...
                // Check for aliasing
                {
                    ...
                    if (hiddenPropertyIndex >= 0)
                    {
                        ...
                        if (TryHidePropertyWithGroup(parentProperty, hiddenProperty, group.Group, includeSpeciallySerializedMembers, out newAliasForHiddenProperty))
                        {
                ...
                if (index >= 0)
                {
                    ...
                    removedMembers.Add(rootMemberProperties[index], group.Group);
                    groupMemberOrders[group.Group] = groupMemberOrders[rootMemberProperties[index]];
                    rootMemberProperties[index] = group.Group;
                }
                else
                {
                    ...
                    rootMemberProperties.Insert(index + 1, group.Group);
                    groupMemberOrders[group.Group] = groupMemberOrders[rootMemberProperties[index]] + 0.1f;
                }
            }
        ...
        private static InspectorPropertyInfo CreatePropertyGroups(InspectorProperty parentProperty, Type typeOfOwner, GroupData groupData, Dictionary<InspectorPropertyInfo, float> memberOrder, bool includeSpeciallySerializedMembers)
        {
            ...
            int order = groupData.ConsolidatedAttribute.Order;

            if (order == 0) order = RecurseGroupMembers(groupData).Min(n => n.Order);

            var result = InspectorPropertyInfo.CreateGroup("#" + groupData.Name, typeOfOwner, order, children.ToArray(), new Attribute[] { groupData.ConsolidatedAttribute });
            return result;
        }
        ...
        private static void PopulateMemberInspectorProperties(InspectorProperty parentProperty, Type type, bool includeSpeciallySerializedMembers, List<InspectorPropertyInfo> properties)
        {
            ...
            {
                PopulateMemberInspectorProperties(parentProperty, type.BaseType, includeSpeciallySerializedMembers, properties);
            }
                ...
                if (InspectorPropertyInfoUtility.TryCreate(parentProperty, member, includeSpeciallySerializedMembers, out info))
                {
                    ...
                    if (previousPropertyWithName != null)
                    {
                        ...
                        if (createAlias)
                        {
                            ...
                            if (InspectorPropertyInfoUtility.TryCreate(parentProperty, alias, includeSpeciallySerializedMembers, out previousPropertyWithName))
                            {
